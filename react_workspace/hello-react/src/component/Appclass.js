...
class Appclass extends Component {
    //생성자 props와 state를 사용하려면 반드시 생성자를 써라
    //props : 부모컴포넌트로부터 자식컴포넌트에 값을 보내기 위한 수단
    //        자식컴포넌트로부터 부모컴포넌트로 값을 보낼 수단은 없다 -> 단방향 컴포넌트

  constructor(props){
    super(props);
    //부모 생성자 호출
    //이 코드는 반드시 생성자의 첫번째 위치에 있어야 한다. 앞에 다른 코드가 올 수 없다
  ...
  render() {
    const {name, age, phone} = this.state; //this.state에 json 개체 저장 -> 값 해체 함수
    //const name = this.state.name; 예전에는 이런식으로 하나씩 해체했어야 함
    //document.getElementById 생략 가능
